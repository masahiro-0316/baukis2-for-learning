...
RSpec.describe StaffMember, type: :model do
  describe "#password=" do
    example "文字列を与えると、hashed_passwordは長さ６０文字列になる" do
      member = StaffMember.new
      member.password = "baukis"
      expect(member.hashed_password).to be_kind_of(String)
      expect(member.hashed_password.size).to eq(60)
    end
    example "nilが与えられると、hashed_passwordなnilになる" do
  ...
  describe "バリデーション" do
    ...
    example "長音符を含むfamily_name_kanaは有効" do
      member = build(:staff_member, family_name_kana: "ジョースター")
      expect(member).to be_valid
    end

    example "他の職員メールアドレスとの重複したemailは無効" do
      member1 = create(:staff_member)
      member2 = build(:staff_member, email: member1.email)
      expect(member2).not_to be_valid
    end

    example "氏名に漢字、ひらがな、カタカナ、アルファベットは有効" do
      member = build(:staff_member, family_name: "To区がワ")
      expect(member).to be_valid
    end
 
    example "氏名に漢字、ひらがな、カタカナ、アルファベット以外は無効" do
      member = build(:staff_member, family_name: "@部")
      expect(member).not_to be_valid
    end
  end
end
